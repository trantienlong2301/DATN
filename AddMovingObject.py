...
class MovingCompositeObject1(QGraphicsObject):
    def __init__(self):
        super().__init__()
...
class MovingCompositeObject(QGraphicsObject):
    def __init__(self,D):
        ...
        # Định nghĩa tọa độ và kích thước cho các hình, theo hệ tọa độ cục bộ của đối tượng.
        self.rect1 = QRectF(-D/2, -D/2, D, D)     
        self.rect2 = QRectF(D/6, -D/12, D/6, D/6)    
        self.rect3 = QRectF(-D/6, -5*D/8, D/3, D/4)  
        self.rect4 = QRectF(-D/6, 3*D/8, D/3, D/4)   
        self.D = D
    ...
    def boundingRect(self):
        ...
        Vùng này phải bao bọc đủ 4 hình (hai hình chữ nhật và hai hình tròn).
        """
        return QRectF(-self.D/2, -self.D*5/8, self.D, self.D*5/4)
...
class GraphicView(QGraphicsView):
    def __init__(self):
        ...
        self.moveObject = MovingCompositeObject(1000)
